...
static std::optional<std::string> ResolveFirstExistingMapTabName(
...
std::optional<std::string> ResolvePreferredMapTabNameForScene(SceneID scene) {
    if (scene == SCENE_TEMPLE_OF_TIME) {
        return ResolveFirstExistingMapTabName({ "ToT", "Temple of Time", "Market", "Overworld" });
    }

    if (IsGanonsTowerRelatedScene(scene)) {
        return ResolveFirstExistingMapTabName({ "Ganons Tower", "Ganon's Tower", "Ganons Castle", "Overworld" });
    }

    if (scene == SCENE_OUTSIDE_GANONS_CASTLE) {
        if (LINK_IS_ADULT) {
            return ResolveFirstExistingMapTabName({ "Ganons Castle", "Ganons Tower", "Hyrule Castle", "Overworld" });
        }
        return ResolveFirstExistingMapTabName({ "Hyrule Castle", "HC", "Overworld" });
    }
...
static void BuildRenderableMarkersByStackKey(
    const MapTabData& tab, bool mqSpoilers, std::vector<std::string>& outStackOrder,
    ...
    for (const auto& marker : tab.markers) {
        ...
        std::string stackKey = BuildMapMarkerStackKey(marker);
        if (!outRenderableMarkersByStackKey.contains(stackKey)) {
            outStackOrder.push_back(stackKey);
        }
        outRenderableMarkersByStackKey[stackKey].push_back(CreateRenderableMapMarker(marker));
    }
}
...
void DrawMapTabContent(MapTabData& tab, bool mqSpoilers) {
    ...
    if (mapClusterPopupState.open && mapClusterPopupState.tabName == tab.mapName) {
        auto popupClusterIt = renderableMarkersByStackKey.find(mapClusterPopupState.stackKey);
...
static std::vector<MapTabVisualSummary> BuildMapTrackerTabVisualSummaries(bool mqSpoilers) {
...
void DrawMapTrackerContent() {
    ...
    std::vector<MapTabVisualSummary> tabVisualSummaries = BuildMapTrackerTabVisualSummaries(mqSpoilers);
